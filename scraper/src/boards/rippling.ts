...
export async function run(db: BetterSQLite3Database, mainLog: L.Log, sampleSaver: C.SampleSaver) {
    ...
    while(true) {
        ...
        const toCheck = C.getCompaniesToCheck(db, Company, [...companiesInProcess], { quota: 2 })
        ...
        await C.sendMessage(
            ...
            db,
            {
                ...
                message: jobInfo.title + ' @ ' + dbJob.companyName + '\n'
                    ...
                    + (Tier.isRequiringClearance(jobInfo.title, longInfo ? C.parseHtml(longInfo.descriptionHtml) : undefined) ? '\n⚠️ clearance?' : '')
            },
        )
    }
